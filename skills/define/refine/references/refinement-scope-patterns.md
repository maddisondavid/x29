...
- Prefer the smallest file set that can credibly improve the active lens concern.
- Read `04-open-questions.md` when uncertainty is likely to affect the lens.
- Read `03-risks.md` when the lens touches operational, delivery, dependency, or trust concerns.
- Read notebook files only when they are likely to sharpen the active lens perspective.
- If `human:` markup appears in relevant files, include those files in the read scope even if they were not part of the default lens pattern.
...
- unrelated Define files outside the active lens concern

## Editing Reminders

- Prefer incremental edits over broad rewrites.
- Preserve unresolved ambiguity as explicit notes.
- If a gap cannot be resolved, route it into `04-open-questions.md` or the lens-relevant acceptance-criteria file.
