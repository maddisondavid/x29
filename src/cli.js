...
function initializeStageDirectories(capabilityRoot) {
...
function initWorkspace(targetDirectory, capabilityName) {
  ...
  ensureDir(path.join(capabilityRoot, "notebook"));
  initializeStageDirectories(capabilityRoot);
...
Use the numbered stage directories to work progressively from definition through execution. Keep loose human notes in \`notebook/\` and keep the stage documents small, reviewable, and easy to refine in parallel.
`
  );

  writeFileIfMissing(
    path.join(capabilityRoot, "notebook", "README.md"),
    `# notebook

Use this directory for loose human notes, meeting notes, stakeholder feedback, and other working inputs that should remain visible without becoming the structured source of truth for a stage.
`
  );
